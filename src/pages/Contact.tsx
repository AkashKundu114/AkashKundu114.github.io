...
export default function Contact() {
  ...
  const handleSubmit = async e => {
    ...
    try {
      ...
      if (res.ok) { setStatus('success'); setForm({ name:'', email:'', message:'' }); setTouched({}) }
      else setStatus('error')
    } catch { setStatus('error') }
  }

  const Notice = ({ border, bg, color, children }) => (
    <div style={{ padding:'12px 16px', border:`1px solid ${border}`, background:bg, color, fontSize:'0.84rem', fontFamily:'var(--font-mono)', lineHeight:1.7, marginBottom:'1.2rem', borderRadius:0 }}>
      {children}
    </div>
  )
    ...
    <PageTransition>
      <section className="section" ref={ref}>
        <div className="container">
          ...
          <div style={{ display:'grid', gridTemplateColumns:'1fr 1.5fr', gap:'3.5rem', alignItems:'start' }} id="contact-grid">
              ...
              <div className="card card-hover">
                {contactInfo.map(({ label, value, href }) => (
                  <div key={label} style={{ padding:'12px 18px', borderBottom:'1px solid var(--border)' }}>
                    <div style={{ fontFamily:'var(--font-mono)', fontSize:'9px', fontWeight:700, letterSpacing:'0.12em', textTransform:'uppercase', color:'var(--accent)', marginBottom:'4px' }}>{label}</div>
                    {href
                      ? <a href={href} style={{ fontSize:'13px', fontWeight:500, color:'var(--ink)' }}>{value}</a>
            ...
            {/* Form */}
            <div className="reveal">
              {status==='success'     && <Notice border="var(--accent)"  bg="var(--accent-soft)"  color="var(--ink)">Message sent — I'll get back to you soon.</Notice>}
              {status==='error'       && <Notice border="var(--danger)"  bg="var(--danger-soft)"  color="var(--danger)">Something went wrong. Email me directly at akashkundu7487@gmail.com</Notice>}
              {status==='no-endpoint' && <Notice border="var(--border)"  bg="var(--surface-2)"    color="var(--muted)">No endpoint configured — set <code>VITE_CONTACT_API</code> or <code>VITE_FORMSPREE_ID</code> in .env</Notice>}

              <form onSubmit={handleSubmit} noValidate style={{ display:'flex', flexDirection:'column', gap:'1rem' }}>
                {fields.map(({ name, type, label, ph }) => (
                  <div key={name} style={{ display:'flex', flexDirection:'column', gap:'0.4rem' }}>
                    <label style={{ fontFamily:'var(--font-mono)', fontSize:'9px', fontWeight:700, letterSpacing:'0.12em', textTransform:'uppercase', color:'var(--muted-2)' }}>{label}</label>
                    ...
                    {errors[name]&&touched[name] && <span style={{ fontSize:'0.7rem', color:'var(--danger)', fontFamily:'var(--font-mono)' }}>{errors[name]}</span>}
                  </div>
                ))}

                <div style={{ display:'flex', flexDirection:'column', gap:'0.4rem' }}>
                  <label style={{ fontFamily:'var(--font-mono)', fontSize:'9px', fontWeight:700, letterSpacing:'0.12em', textTransform:'uppercase', color:'var(--muted-2)' }}>Message</label>
                  ...
                  {errors.message&&touched.message && <span style={{ fontSize:'0.7rem', color:'var(--danger)', fontFamily:'var(--font-mono)' }}>{errors.message}</span>}
                </div>
