    ...
    <PageTransition>
      <section className="section" style={{ paddingTop:'9rem' }} ref={ref}>
        ...
        <div className="container">
          ...
          <h2 className="reveal" style={{ marginBottom:'4rem' }}>A little about<br /><em style={{ color:'var(--accent)',fontStyle:'normal' }}>who I am.</em></h2>
          <div className="about-grid" style={{ display:'grid',gridTemplateColumns:'1fr 1.6fr',gap:'5rem',alignItems:'start' }}>
            <div className="reveal" style={{ background:'var(--surface)',border:'1px solid var(--border)',borderRadius:'4px',overflow:'hidden' }}>
              <div style={{ background:'var(--accent)',padding:'1.8rem',display:'flex',justifyContent:'center' }}>
                <div style={{ width:72,height:72,borderRadius:'50%',background:'#05080f',display:'flex',alignItems:'center',justifyContent:'center',fontFamily:'Syne,sans-serif',fontSize:'1.7rem',fontWeight:800,color:'var(--accent)' }}>AK</div>
              </div>
              <div style={{ padding:'1.2rem' }}>
                {metaRows.map(({ key,val,link,accent }) => (
                  <div key={key} className="flex justify-between items-start gap-3" style={{ padding:'0.72rem 0',borderBottom:'1px solid var(--border)' }}>
                    ...
                    {link ? <a href={link} style={{ fontSize:'0.78rem',fontWeight:500,textAlign:'right',wordBreak:'break-all',color:'var(--ink)' }}>{val}</a>
            ...
            <div>
              <p className="reveal" style={{ fontSize:'1rem',lineHeight:1.8,color:'var(--muted)',marginBottom:'1.2rem' }}>Pursuing a B.Tech in Computer Science at Techno India University. Eager to apply analytical and creative problem-solving skills in a professional setting, seeking internships that offer hands-on experience with real-world data challenges while contributing innovative insights to support organisational goals.</p>
              <p className="reveal" style={{ fontSize:'1rem',lineHeight:1.8,color:'var(--muted)' }}>Interested in Data Analytics with a focus on extracting insights from data, visualisation, and building data-driven solutions. Skilled in programming and modern development tools with experience in AI-driven solutions, responsive applications, collaborative workflows, and deployment practices.</p>
              <div className="reveal" style={{ display:'grid',gridTemplateColumns:'repeat(3,1fr)',gap:'0.8rem',marginTop:'2rem' }}>
                {strengths.map(({ icon,title,desc }) => (
                  <div key={title} style={{ padding:'1.2rem',background:'var(--surface)',border:'1px solid var(--border)',borderRadius:'4px',transition:'all 0.25s' }}
