...
const ticker = [
  'Neural Network','Memory Nodes','Logic Gates','Prompt Forge',
  'Pattern Recognition','Data Synthesis','Anomaly Detection',
...
function NetCanvas() {
  ...
  useEffect(() => {
    ...
    const draw = () => {
      ...
      nodes.forEach(n => {
        const a = 0.20 + Math.sin(t + n.ph) * 0.08
        ctx.beginPath()
      ...
      <div ref={ref}>
          ...
          <div className="container" style={{ position:'relative', zIndex:1, width:'100%' }}>
            ...
            <div className="reveal" style={{ marginBottom:'28px' }}>
              <span style={{
                ...
                <span style={{ width:5, height:5, borderRadius:'50%', background:'var(--success)', animation:'pulse-dot 2.4s ease-in-out infinite', flexShrink:0 }} />
            ...
            <p className="reveal" style={{
              fontSize:'15px', lineHeight:1.80, maxWidth:'500px', marginBottom:'36px',
              fontFamily:'var(--font-mono)', color:'var(--muted)',
            }}>
              B.Tech CSE student at Techno India University, Kolkata — building across
              the full stack from ML model to production UI.
            </p>

            <div className="reveal" style={{ display:'flex', flexWrap:'wrap', gap:'10px', marginBottom:'52px' }}>
              ...
              <a href="/cv/AkashKundu_Resume.pdf" download className="btn btn-lg">Resume ↓</a>
            </div>

            {/* Stats row */}
            <div className="reveal" style={{ display:'flex', flexWrap:'wrap', gap:'44px', marginBottom:'44px' }} id="stats-row">
              {stats.map(s => (
              ...
              <div className="marquee-track">
                {[...ticker, ...ticker].map((node, i) => (
                  <span key={node+i} className="marquee-item" style={{
                    color: i % 3 === 0 ? 'var(--link)' : i % 3 === 1 ? 'var(--accent)' : 'var(--muted-2)',
                  }}>
        ...
        <section className="section">
          <div className="container">
            ...
            <h2 className="reveal" style={{ marginBottom:'32px', maxWidth:'26ch' }}>Three layers, every project.</h2>
            <div className="bento-grid stagger" style={{ gridTemplateColumns:'repeat(3,1fr)' }} id="domain-grid">
              {domains.map((d, i) => (
                <div key={d.label} className="card card-hover reveal" style={{ padding:'28px 24px', '--i':i }}>
                  <div style={{ display:'flex', alignItems:'center', gap:'10px', marginBottom:'16px' }}>
                    <span style={{
                      display:'inline-flex', alignItems:'center', justifyContent:'center',
                      width:'30px', height:'30px', fontSize:'14px', color:d.color,
                      border:`1px solid ${d.color}28`, background:`${d.color}0c`, borderRadius:0,
                    }}>{d.icon}</span>
                    <h3 style={{ color:'var(--ink)' }}>{d.label}</h3>
                  </div>
                  <p style={{ fontSize:'13px', lineHeight:1.75, marginBottom:'20px' }}>{d.desc}</p>
        ...
        <section className="section">
          <div className="container">
            ...
            <div className="stagger" style={{ display:'flex', flexDirection:'column', gap:'8px' }}>
              {featured.map((p, i) => (
                <div key={p.id} className="row-card reveal"
                  ...
                  <div style={{ display:'flex', alignItems:'flex-start', justifyContent:'space-between', gap:'16px', flexWrap:'wrap' }}>
                    <div style={{ flex:1, minWidth:240 }}>
                      <div style={{ display:'flex', alignItems:'center', gap:'8px', marginBottom:'8px' }}>
                        <span style={{ fontFamily:'var(--font-mono)', fontSize:'10px', color:'var(--muted-2)', letterSpacing:'0.08em' }}>{p.year}</span>
                        {p.status && <span className="tag">{p.status}</span>}
                      </div>
                      <h3 style={{ marginBottom:'6px', fontSize:'0.95rem', color:'var(--ink)' }}>{p.title}</h3>
                      <p style={{ fontSize:'13px', lineHeight:1.65, maxWidth:'54ch' }}>{p.shortDesc}</p>
                    </div>
                    <div style={{ display:'flex', flexWrap:'wrap', gap:'4px', maxWidth:'220px', justifyContent:'flex-end' }}>
        ...
        <section className="section" style={{ background:'var(--surface)', borderTop:'1px solid var(--border)' }}>
          <div className="container">
            <div className="reveal" style={{
              ...
              borderLeft:'2px solid var(--accent)', paddingLeft:'24px',
            }}>
              <div>
              ...
              <div style={{ display:'flex', gap:'10px', flexWrap:'wrap' }}>
                ...
                <a href="https://www.linkedin.com/in/akashkundu114/" target="_blank" rel="noopener noreferrer" className="btn">LinkedIn ↗</a>
              </div>
            </div>
          </div>
        </section>
