          ...
          <div style={{ position:'relative', maxWidth:'90vw' }} onClick={e => e.stopPropagation()}>
            ...
            {project.screenshots.length > 1 && (
              <div style={{ display:'flex', justifyContent:'space-between', marginTop:'14px', gap:'12px' }}>
                ...
                <span style={{ fontFamily:'var(--font-mono)', fontSize:'10px', color:'rgba(254,250,239,0.5)', alignSelf:'center' }}>
      ...
      <section style={{ paddingTop: '88px' }} ref={ref}>
        <div className="container">
          ...
          <div className="reveal" style={{ marginBottom:'32px' }}>
            ...
            <h1 style={{ fontSize:'clamp(1.6rem,4vw,2.4rem)', marginBottom:'16px', lineHeight:1.08 }}>{project.title}</h1>
          ...
          <div style={{ display:'grid', gridTemplateColumns:'minmax(0,1.5fr) minmax(0,1fr)', gap:'20px', alignItems:'start' }} id="detail-grid">
            <div>
              ...
              <div className="reveal">
                ...
                {hasScreenshots ? (
                  <div className="stagger" style={{ display:'grid', gridTemplateColumns:'repeat(auto-fill,minmax(200px,1fr))', gap:'8px' }}>
                    {project.screenshots.map((shot,i) => (
                      <div key={i} className="card card-hover reveal" style={{ overflow:'hidden', cursor:'none', '--i':i }} onClick={() => setLightbox(i)}>
                        <img src={shot.src} alt={shot.caption} style={{ width:'100%', aspectRatio:'16/9', objectFit:'cover', display:'block' }} />
                        <div style={{ padding:'8px 12px' }}>
                          <span style={{ fontFamily:'var(--font-mono)', fontSize:'10px', color:'var(--muted-2)' }}>{shot.caption}</span>
                        </div>
                      </div>
                    ))}
                  </div>
