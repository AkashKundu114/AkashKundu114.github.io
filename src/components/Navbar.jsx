    ...
    >
      <div className="container">
        <div className="flex items-center justify-between">
          ...
          <Link
            to="/"
            className="font-display font-extrabold text-lg tracking-tight text-ink hover:text-ink"
          ...
          <ul className="hidden md:flex items-center gap-0.5 list-none">
            {navLinks.map(({ to, label }) => {
              ...
              return (
                <li key={to}>
                  <Link
                    to={to}
                    className="block px-3 py-1.5 text-xs font-medium tracking-widest uppercase rounded transition-all duration-200"
                    style={{
            ...
            <li>
              <a
                ...
                className="inline-flex items-center gap-1.5 px-3 py-1.5 border rounded text-xs font-semibold uppercase tracking-wider transition-all duration-200"
                style={{
          ...
          <div className="flex md:hidden items-center gap-2">
            <button
              onClick={toggleTheme}
              className="flex items-center justify-center w-8 h-8 rounded border font-mono text-base"
              style={{ background: 'var(--surface2)', borderColor: 'var(--border)', color: 'var(--ink)' }}
            >
              {theme === 'dark' ? '☀' : '☾'}
            </button>
        ...
        {open && (
          <div
            className="md:hidden border-t mt-2 pt-2 pb-3 flex flex-col gap-0.5"
          ...
          >
            {navLinks.map(({ to, label }) => (
              <Link
                key={to}
                to={to}
                className="block px-3 py-2 text-xs font-medium tracking-wider uppercase rounded transition-all duration-200"
                style={{
            ...
            <a
              ...
              target="_blank"
              rel="noopener noreferrer"
              className="block px-3 py-2 text-xs font-medium tracking-wider uppercase"
