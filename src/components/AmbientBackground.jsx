...
/* Floating particle dust — Powder Blue on Deep Blue.
   The mouse-follow blob is now a canvas element here. */
export default function AmbientBackground() {
  ...
  useEffect(() => {
    ...
    const frame = () => {
      ...
      // Powder Blue motes on Deep Blue background
      motes.forEach(m => {
    ...
    const draw = () => { t += 0.3; frame(); animId = requestAnimationFrame(draw) }
    if (reduceMotion) frame(); else draw()
